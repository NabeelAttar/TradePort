...
export const getRecommendedProducts = async (req: any, res: Response, next: NextFunction) => {
    try {
        ...
        let userAnalytics = await prisma.userAnalytics.findUnique({
            where: { userId },
            select: { actions: true, recoemmendations: true, lastTrained: true },
        })
        ...
        if(!userAnalytics){
            ...
            const recommendations = Array.isArray(userAnalytics.recoemmendations) ? (userAnalytics.recoemmendations as string[]) : []
            ...
            if(actions.length < 50){
                ...
                recommendedProducts = products.filter((product:any) => recommendations.includes(product.id))
            } else {
                const recommendedProductIds = await recommendProducts(userId, products)
                recommendedProducts = products.filter((product:any) => recommendedProductIds.includes(product.id))
