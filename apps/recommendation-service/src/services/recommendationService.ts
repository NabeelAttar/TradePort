...
import { getUserActivity } from "./fetchUserActivity"
...
export const recommendProducts = async (userId: string, allProducts: any): Promise<string[]> => {
    ...
    // understand relation between two model input layers
    // create a embedding layer like lookup tables to learn the relationships
    const userEmbedding = tf.layers.embedding({ 
        inputDim: userCount, 
        outputDim: EMBEDDING_DIM 
    }).apply(userInput) as tf.SymbolicTensor
    // with above line of code we say create a dense vector for each userCount of 50 dimensions
    // it will capture similarity in the behaviour - this is called call of duty filtering model
    ...
    // final layer : outputs the probability of the interactions
    ...
    // convert user and producct interactions into tensors for training
    ...
    const recommendProducts: RecommendedProduct[] = Object.keys(productMap).map((productId, index) => ({
        productId,
