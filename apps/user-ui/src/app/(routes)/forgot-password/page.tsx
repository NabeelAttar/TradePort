...
const ForgotPassword = () => {
  ...
  const resetPasswordMutation = useMutation({
    ...
    onSuccess: () => {
      setStep("email"),
      toast.success("Password reset was successful. Please login with your new password.");
      setServerError(null);
      router.push("/login");
    },
    onError: (error: AxiosError) => {
  ...
  const handleOtpChange = (index: number, value: string) => {
    ...
    if(value && index < inputRefs.current.length - 1){
      inputRefs.current[index+1]?.focus(); //moves cursor to next box
    }
  }

  const handleOtpKeyDown = (index: number, e:React.KeyboardEvent<HTMLInputElement>) => {
    if(e.key === 'Backspace' && !otp[index] && index > 0){
      inputRefs.current[index - 1]?.focus(); //if from current box pressed backspace key then cursor goes to previous box
    }
  }
          ...
          {step==="email" && (
            <>
              ...
              <form onSubmit={handleSubmit(onSubmitEmail)}>
                ...
                <input type="email" 
                placeholder='nabeelattar2411@gmail.com'
                ...
                <button type='submit' disabled={requestOtpMutation.isPending} className='w-full cursor-pointer mt-4 bg-black text-white py-2 rounded-lg'>
                ...
                <div className='flex justify-center gap-6'>
                  {otp.map((digit, index) => (
                    <input 
                      ...
                      /> 
                      // inputrefs ke wajah se agar currentIndex pe otp ki value daal di to cursor apne aap next box pe jaata 
                    ))}
                </div>
                <button 
...
// installed react-hook-form and tanstack query which is used for fetching requests or data and sending to apiroutes