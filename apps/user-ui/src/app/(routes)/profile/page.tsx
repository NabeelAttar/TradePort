...
const page = () => {
    ...
    const markAsRead = async (notificationId: string) => {
        await axiosInstance.post("/admin/api/mark-notification-as-read", { notificationId })
    }
                                ...
                                {!isLoading && notifications?.length > 0 && (
                                    <div className='md:w-[80%] my-6 rounded-lg divide-y divide-gray-800 bg-black/40 backdrop-blur-lg shadow-sm'>
                                        {notifications.map((d: any) => (
                                            <Link key={d.id} href={d.redirect_link} className={`block px-5 py-4 transition ${d.status !== "Unread" ? "hover:bg-gray-800/40" : "bg-gray-800/50 hover:bg-gray-800/70"}`} onClick={() => markAsRead(d.id)}>
                                                <div className='flex items-start gap-3'>
                                                    <div className='flex flex-col'>
                                                        <span className='text-white font-medium'>{d.title}</span>
                                                        <span className='text-gray-300 text-sm'>{d.messsage}</span>
