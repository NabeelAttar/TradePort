...
const CheckoutForm = ({ clientSecret, cartItems, coupon, sessionId } : {clientSecret : string, cartItems : any[], coupon : any, sessionId: string | null}) => {
    ...
    const total = cartItems.reduce((sum, item) => sum + item.quantity * item.sale_price, 0)
    ...
    return (
        <div className='flex justify-center items-center min-h-[80vh] px-4 my-10'>
            <form className='bg-white w-full max-w-lg p-8 rounded-md shadow space-y-6' onSubmit={handleSubmit}>
                ...
                <PaymentElement />
                <button type='submit' disabled={!stripe || loading} className='w-full bg-blue-600 text-white py-2 rounded-lg font-medium hover:bg-blue-700'>
                ...
                {status === "failed" && (
                    <div className='flex items-center gap-2 text-red-600 text-sm justify-center'>
                        <XCircle className='w-5 h-5' />
                        Paymnet Failed! Please try again.
                    </div>
                )}
            </form>
        </div>
